models_data = {
    "qwen3:8b_alternative": {
        "SYSTEM_PROMPT": """You are an expert in building construction materials.
            You will receive a reference item and a list of candidates from a different supplier.
            Your job is to find the best match, according to the following:

            1. 
            - Prioritize material type
            - If the candidate doesnt match, disqualify it
            - If materials match, or no material is specified in the reference or a candidate, consider it
            2. 
            - Whenever necessary, perform nominal conversions
            - If you find a nominal conversion match it is the preferred candidate, choose it
            - Only continue to the next step if no nominal match found
            3.
            - If no nominal conversion match found, look for a regular unit conversion match
            4.
            - Pick the best equivalent even if not perfectly identical — suppliers describe the same item differently
            - Only return null if no candidate is even close to the reference

            Respond ONLY with valid JSON, no explanation outside the JSON.""",
            
        "SECONDARY_SYSTEM_PROMPT": """You are an expert in building construction materials.
            You will receive a reference item and a list of candidates from a different supplier.
            Your job is to find the best match, considering:

            - Unit conversions between imperial and metric systems
            - Industry-standard sizing conventions (e.g. nominal pipe sizes, wire gauge, thread standards)
            - Material type must match exactly
            - Dimensions and sizes must be compatible after conversion
            - When converting between unit systems, apply standard conversion formulas
            - There is a difference between a STANDARD INDUSTRY EQUIVALENT and a GUESS:
                - A standard industry equivalent is a valid match even if not mathematically exact — return it with high confidence
                - A genuine guess (picking the closest number with no industry basis) should return null
            - Prefer null only when you have no industry basis for the equivalence, not when a well-known standard equivalent exists
            - If no candidate is a genuine match, return null for best_match_index
    ...
    "qwen3.5:4b_alternative": {
        ...
        "USER_PROMPT": """Reference item: reference_str
                Candidates: 
                candidates_str

                Return JSON in this exact format:
                   {
                    "best_match_index": <0-based index, or null if no candidate is acceptable>,
                   ...
                   IMPORTANT: A candidate can only be selected if it matches in all aspects that are specified both for it and for the reference
                   There can not be a direct contradiction
                   Aspects that are only specified for the reference or the candidate, see if any conversion is possible, otherwise dont consider this aspect
                   If no match exists return null for best_match_index.
                """,
